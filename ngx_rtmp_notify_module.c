...
static ngx_chain_t *
ngx_rtmp_notify_pause_create(ngx_rtmp_session_t *s, void *arg, 
        ngx_pool_t *pool)
{
    ngx_rtmp_pause_t               *v = arg;
    ...
    b = ngx_create_temp_buf(pool,
            ...
            sizeof("&addr=") + addr_text->len + 
            sizeof("&name=") + name_len * 3 +
            sizeof("&position=&pause=") + NGX_OFF_T_LEN * 2
            + 1 + args_len);
    if (b == NULL) {
        return NULL;
    }
    ...
    b->last = ngx_cpymem(b->last, (u_char*)"&call=pause", 
            sizeof("&call=pause") - 1);
    ...
    /* HTTP header */
    hl = ngx_rtmp_netcall_http_format_header(nacf->pause_url, pool,
            cl->buf->last - cl->buf->pos + (pl->buf->last - pl->buf->pos),
            &ngx_rtmp_netcall_content_type_urlencoded);
...
static ngx_chain_t *
ngx_rtmp_notify_seek_create(ngx_rtmp_session_t *s, void *arg, 
        ngx_pool_t *pool)
{
    ngx_rtmp_seek_t                *v = arg;
    ...
    b->last = ngx_cpymem(b->last, (u_char*)"&call=seek", 
            sizeof("&call=seek") - 1);
    ...
    /* HTTP header */
    hl = ngx_rtmp_netcall_http_format_header(nacf->seek_url, pool,
            cl->buf->last - cl->buf->pos + (pl->buf->last - pl->buf->pos),
            &ngx_rtmp_netcall_content_type_urlencoded);
...
static ngx_chain_t *
ngx_rtmp_notify_close_create(ngx_rtmp_session_t *s, void *arg, 
        ngx_pool_t *pool)
{
    ngx_rtmp_notify_app_conf_t     *nacf;
    ngx_chain_t                    *hl, *cl, *pl;
    ...
    b->last = ngx_cpymem(b->last, (u_char*)"&call=close", 
            sizeof("&call=close") - 1);
    ...
    /* HTTP header */
    hl = ngx_rtmp_netcall_http_format_header(nacf->close_url, pool,
            cl->buf->last - cl->buf->pos + (pl->buf->last - pl->buf->pos),
            &ngx_rtmp_netcall_content_type_urlencoded);
...
static ngx_int_t
ngx_rtmp_notify_pause(ngx_rtmp_session_t *s, ngx_rtmp_pause_t *v)
{
    ngx_rtmp_notify_app_conf_t     *nacf;
    ngx_rtmp_netcall_init_t         ci;

    nacf = ngx_rtmp_get_module_app_conf(s, ngx_rtmp_notify_module);
    if (nacf == NULL || nacf->pause_url == NULL) {
        goto next;
    }

    ngx_memzero(&ci, sizeof(ci));
    ci.url = nacf->pause_url;
    ci.create = ngx_rtmp_notify_pause_create;
    ci.arg = v;
    ci.argsize = sizeof(*v);
...
static ngx_int_t
ngx_rtmp_notify_seek(ngx_rtmp_session_t *s, ngx_rtmp_seek_t *v)
{
    ngx_rtmp_notify_app_conf_t     *nacf;
    ngx_rtmp_netcall_init_t         ci;

    nacf = ngx_rtmp_get_module_app_conf(s, ngx_rtmp_notify_module);
    if (nacf == NULL || nacf->seek_url == NULL) {
        goto next;
    }

    ngx_memzero(&ci, sizeof(ci));
    ci.url = nacf->seek_url;
    ci.create = ngx_rtmp_notify_seek_create;
    ci.arg = v;
    ci.argsize = sizeof(*v);
...
static ngx_int_t
ngx_rtmp_notify_close_stream(ngx_rtmp_session_t *s, ngx_rtmp_close_stream_t 
        *v)
{
    ngx_rtmp_notify_app_conf_t     *nacf;
    ngx_rtmp_netcall_init_t         ci;
    ...
    ngx_memzero(&ci, sizeof(ci));
    ci.url = nacf->close_url;
    ci.create = ngx_rtmp_notify_close_create;
...
ngx_rtmp_notify_postconfiguration(ngx_conf_t *cf)
{
    ...
    ngx_rtmp_pause = ngx_rtmp_notify_pause;

    next_seek = ngx_rtmp_seek;
    ngx_rtmp_seek = ngx_rtmp_notify_seek;

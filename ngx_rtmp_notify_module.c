...
enum {
    ...
    NGX_RTMP_NOTIFY_PAUSE,
    NGX_RTMP_NOTIFY_SEEK,
        ...
        ngx_pool_t *pool)
{
    ...
    b = ngx_create_temp_buf(pool,
                            sizeof("&call=pause") +
                            sizeof("&name=") + name_len * 3 +
                            sizeof("&position=&pause=") +
                            NGX_OFF_T_LEN * 2 + 1 + args_len);
    if (b == NULL) {
        return NULL;
    }
    ...
    return ngx_rtmp_notify_create_request(s, pool, NGX_RTMP_NOTIFY_PAUSE, pl);
}
        ...
        ngx_pool_t *pool)
{
    ...
    return ngx_rtmp_notify_create_request(s, pool, NGX_RTMP_NOTIFY_SEEK, pl);
}
...
ngx_rtmp_notify_pause(ngx_rtmp_session_t *s, ngx_rtmp_pause_t *v)
{
    ...
    url = nacf->url[NGX_RTMP_NOTIFY_PAUSE];
    if (url == NULL) {
        goto next;
    }

    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                  "notify: pause '%V'", &url->url);
...
ngx_rtmp_notify_seek(ngx_rtmp_session_t *s, ngx_rtmp_seek_t *v)
{
    ...
    url = nacf->url[NGX_RTMP_NOTIFY_SEEK];
    if (url == NULL) {
        goto next;
    }

    ngx_log_error(NGX_LOG_INFO, s->connection->log, 0,
                  "notify: seek '%V'", &url->url);
    ...
    switch (name->len) {
        case sizeof("on_done") - 1: /* and on_play */
            if (name->data[3] == 'd') {
                ...
                n = NGX_RTMP_NOTIFY_SEEK;
            } else {
                n = NGX_RTMP_NOTIFY_PLAY;
            }
            break;

        case sizeof("on_pause") - 1:
            n = NGX_RTMP_NOTIFY_PAUSE;
            break;
